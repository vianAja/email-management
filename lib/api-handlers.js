...
const CLIENT_ID =
  process.env.GOOGLE_OAUTH_CLIENT_ID || process.env.CLIENT_ID_GMAIL_API || '';
const CLIENT_SECRET =
  process.env.GOOGLE_OAUTH_CLIENT_SECRET || process.env.SECRET_GMAIL_API || '';
...
const getSessionSecret = (request) => {
  ...
  throw new HttpError(
    500,
    'SESSION_SECRET belum dikonfigurasi. Tambahkan secret ini di environment Vercel.'
  );
};
...
const ensureOAuthConfigured = () => {
  if (!CLIENT_ID || !CLIENT_SECRET) {
    throw new HttpError(
      500,
      'Google OAuth belum dikonfigurasi. Lengkapi GOOGLE_OAUTH_CLIENT_ID dan GOOGLE_OAUTH_CLIENT_SECRET.'
    );
  }
};
...
const getAuthorizedState = async (request) => {
  ...
  try {
    ...
    session.tokens = normalizeTokens(auth.credentials, session.tokens);
    auth.setCredentials(session.tokens);
